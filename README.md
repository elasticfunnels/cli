...
| `ef pages events push <slug>` | Push `pages/<slug>.events.json` (validates first; `--strict` blocks on errors). Mints a `node_code` for any split-test node missing one — the server reads that field but never creates it, and without it a test splits traffic correctly while reporting its arms as `j:null`. Existing codes are never rewritten (a live test's sessions are tied to them). **Refuses (exit 4)** if the server changed since you pulled, or if you never pulled a page that already has events (always-pull-first); `--force` overwrites. No auto-merge (structured JSON). |
